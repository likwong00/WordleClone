...
export function checkGuess(guess: string, answer: string) {
  const res: TileState[] = Array.from({ length: guess.length }, () => 'absent')
...
export function useGame(): UseGame {
  ...
  const [answer, setAnswer] = useState<string>(() => pickAnswer(defaultWords))
  const [wordLength] = useState(5)
  const [board, setBoard] = useState<string[]>(() => Array.from({ length: maxGuesses }, () => ''))
  const [states, setStates] = useState<TileState[][]>(() => Array.from({ length: maxGuesses }, () => Array.from({ length: wordLength }, () => 'empty' as TileState)))
  ...
  // Save state helper
  const saveState = useCallback((next?: Partial<Record<string, any>>) => {
    try {
      const payload = {
        board,
        states,
      ...
      localStorage.setItem('wordle-clone-state', JSON.stringify(payload))
    } catch (e) {}
  }, [board, states, currentRow, currentGuess, answer, maxGuesses, wordList, gameOver, lastResult])
    ...
    for (let r = 0; r < states.length; r++) {
      ...
      for (let i = 0; i < rowStates.length; i++) {
        ...
        if (st === 'correct') {
          ...
          if (prev !== 'correct' && prev !== 'present') map[ch] = 'present'
        } else {
          if (!prev) map[ch] = st
        }
      }
  ...
  const reset = useCallback(() => {
    ...
    const emptyStates = Array.from({ length: maxGuesses }, () => Array.from({ length: wordLength }, () => 'empty' as TileState))
    setBoard(emptyBoard)
    ...
    setGameOver(false)
    setLastResult(null)
    saveState({ answer: nextAnswer, board: emptyBoard, states: emptyStates, currentRow: 0, currentGuess: '', gameOver: false, lastResult: null })
  ...
  const submitGuess = useCallback(() => {
    ...
    if (isWin) {
      ...
      setStats((st: any) => {
        ...
        try { localStorage.setItem('wordle-clone-stats', JSON.stringify(next)) } catch (e) {}
        return next
      })
      saveState({ board: newBoard, states: newStates, currentRow: nextRow, currentGuess: '', gameOver: true, lastResult: 'win' })
      return
    }
    ...
    if (nextRow >= maxGuesses) {
      ...
      saveState({ board: newBoard, states: newStates, currentRow: nextRow, currentGuess: '', gameOver: true, lastResult: 'lose' })
      return
    }

    saveState({ board: newBoard, states: newStates, currentRow: nextRow, currentGuess: '' })
  ...
  const setConfig = useCallback((opts: { maxGuesses?: number; words?: string[] }) => {
    if (opts.maxGuesses) setMaxGuesses(opts.maxGuesses)
    if (opts.words) setWordList(opts.words.map((w) => w.toLowerCase()))
    // persist new config
    saveState({ maxGuesses: opts.maxGuesses, wordList: opts.words ? opts.words.map((w) => w.toLowerCase()) : undefined })
  }, [])
