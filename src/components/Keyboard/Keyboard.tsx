...
export function Keyboard({
    ...
    const renderKey = (letter: string) => {
        ...
        return (
            <button key={letter} onClick={() => keyOnClick(letter)} className={className}>
                {letter}
            </button>
        );
    };
    ...
    return (
        <div className="keyboard" data-testid="keyboard-root">
            ...
            <div className="keyboard-row">
                <button className="key-wide" onClick={submitGuess} aria-label="Enter">
                    ENTER
                </button>

                {Array.from(ROWS[2]).map((letter) => renderKey(letter))}

                <button className="key-wide" onClick={backspace} aria-label="Backspace">
